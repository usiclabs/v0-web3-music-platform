-- Eliza Agent Tables
-- Run this script to create the necessary tables for autonomous Eliza agents
...
-- Agent wallet mappings (reuses existing mm_agent_wallets structure)
-- No new table needed, we use the existing generateWalletsForAgent function
...
CREATE POLICY eliza_agents_owner_policy ON eliza_agents
  FOR ALL
  USING (owner_address = current_setting('request.jwt.claim.sub', true));

CREATE POLICY eliza_memory_owner_policy ON eliza_memory
  FOR ALL
  USING (agent_id IN (
    SELECT id FROM eliza_agents WHERE owner_address = current_setting('request.jwt.claim.sub', true)
  ));

CREATE POLICY eliza_actions_owner_policy ON eliza_actions
  FOR ALL
  USING (agent_id IN (
    SELECT id FROM eliza_agents WHERE owner_address = current_setting('request.jwt.claim.sub', true)
  ));
...
CREATE TRIGGER eliza_agent_updated_at_trigger
