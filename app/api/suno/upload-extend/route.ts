...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!response.ok) {
      ...
      console.error("Suno upload-extend API error:", errorText)
      return NextResponse.json({ error: "Failed to extend audio" }, { status: response.status })
    }

    const data = await response.json()

    if (data.code !== 200) {
      return NextResponse.json({ error: data.msg || "Extension failed" }, { status: 400 })
    }
