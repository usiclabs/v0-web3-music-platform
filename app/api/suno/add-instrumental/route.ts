...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${SUNO_API_BASE}/generate/add-instrumental`, {
      ...
      body: JSON.stringify({
        uploadUrl,
        prompt: prompt || "Create an instrumental accompaniment",
        style: style || "Pop",
    ...
    if (!response.ok) {
      ...
      console.error("Suno add-instrumental API error:", errorText)
      return NextResponse.json({ error: "Failed to add instrumental" }, { status: response.status })
    }

    const data = await response.json()

    if (data.code !== 200) {
      return NextResponse.json({ error: data.msg || "Instrumental generation failed" }, { status: 400 })
    }
