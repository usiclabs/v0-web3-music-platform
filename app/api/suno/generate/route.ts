...
export async function POST(request: NextRequest) {
  try {
    ...
    const { prompt, title, style, instrumental, model = "V4" } = body
    ...
    console.log("[v0] Suno generate request with callback URL:", callBackUrl)
    ...
    const response = await fetch(`${SUNO_API_BASE}/generate`, {
      ...
      body: JSON.stringify({
        ...
        model,
        callBackUrl, // Added required callback URL parameter
      }),
    })
